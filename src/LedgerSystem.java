...
public class LedgerSystem_Copyy{
   ...
   private static void loginUser() {
      ...
      try (Connection conn = DriverManager.getConnection(URL, USER, PASSWORD)) {
         ...
         if (resultSet.next()) {
            ...
            System.out.println("Balance : " + displayBalance()); //lead to 3a
            reminder();//reminder user there is unpaid loan
   ...
   private static double displayBalance() {
      try (Connection conn = DriverManager.getConnection(URL, USER, PASSWORD)) {
   ...
   private static void viewTransactionHistory() {
      ...
      try (Connection conn = DriverManager.getConnection(URL, USER, PASSWORD); //This one idk why, but because now no error, thats why nothing happens. 
         ...
         System.out.printf("%-10s | %-35s | %-9s | %-9s | %-9s\n", "Date", "Description", "Debit", "Credit", "Balance");
   ...
   private static void depositInterestPredictor() {
         ...
         while (true) {
            try {
               ...
               while (rs.next()) {
                     ...
                     double savingRate = rs.getDouble("saving_rate");
                     System.out.printf("%s. %s: %.2f%%%n", bankID, bankName, savingRate);
               }
               ...
               query = "SELECT saving_rate FROM Banks WHERE bank_id = ?";
               stmt = conn.prepareStatement(query);
               stmt.setString(1, selectedBank);
               rs = stmt.executeQuery();
               if (rs.next()) {
                     interestRate = rs.getDouble("saving_rate");
               } else {
      ...
      //check whether there is a overdue loan
      static boolean checkOverDue(){
         try (Connection conn = DriverManager.getConnection(URL, USER, PASSWORD)){
            String query = "SELECT due_date, status FROM Loans WHERE user_id = ?";
            ...
            while(rs.next()){
               ...
               //check whether due ald or not
               if(currentDate.isAfter(dueDate) && status.equalsIgnoreCase("active")){
                  return true;
               }
            }
      ...
      static void reminder(){
         if(hasUnpaidLoan()){
            System.out.println("REMINDER THERE IS UNPAID LOAN UNDER YOUR BANK ACCOUNT !");
         }
      }

      static void displayLoanInterest(){
         try (Connection conn = DriverManager.getConnection(URL, USER, PASSWORD)){
            String bankQuery = "SELECT * FROM BANKS";
            PreparedStatement statementGet = conn.prepareStatement(bankQuery);
            ResultSet rs = statementGet.executeQuery();
            
            while (rs.next()) {
               int bankID = rs.getInt("bank_id");
               String bankName = rs.getString("bank_name");
               double loanRate = rs.getDouble("loan_rate");
               System.out.printf("%s. %s: %.2f%%%n", bankID, bankName, loanRate);
         }
   ...
   static void applyLoan(){
      try (Connection conn = DriverManager.getConnection(URL, USER, PASSWORD)){
            ...
            System.out.print("Enter the amount you wish to apply for: ");
            double loanAmount = scanner.nextDouble();

            System.out.println("Which bank do you wish to take loan from?");
            displayLoanInterest();  // Assuming this method is defined elsewhere

            int bankId = scanner.nextInt();

            // Query to get the interest rate from the selected bank
            String interestQuery = "SELECT loan_rate FROM BANKS WHERE bank_id = ?";
            PreparedStatement statementInterest = conn.prepareStatement(interestQuery);
            statementInterest.setInt(1, bankId);
            ResultSet rs = statementInterest.executeQuery();

            double rate = 0.0;
            if (rs.next()) { // Check if there is a result
               rate = rs.getDouble("loan_rate"); // Retrieve the value of loan_rate
            }

            // Get the repayment period from user
            System.out.print("Enter the repayment period (in months): ");
            int loanPeriod = scanner.nextInt();

            // Calculate the outstanding balance
            double outstanding_balance = loanAmount * (1 + (rate / 100) * (loanPeriod/12));
            LocalDate date = LocalDate.now().plusMonths(loanPeriod);
            java.sql.Date due_date = java.sql.Date.valueOf(date);

            double monthly_payment = outstanding_balance/loanPeriod;
            

            // Insert the loan into the Loans table
            String loanQuery = "INSERT INTO Loans (user_id, principal_amount, interest_rate, repayment_period, outstanding_balance, status, due_date, monthly_payment) VALUES (?, ?, ?, ?, ?, \"active\",? ,?)";
            PreparedStatement statementSet = conn.prepareStatement(loanQuery, PreparedStatement.RETURN_GENERATED_KEYS);  // Get generated keys
            statementSet.setInt(1, userId);  // Assuming `userId` is already defined
            statementSet.setDouble(2, loanAmount);
            statementSet.setDouble(3, rate);
            statementSet.setInt(4, loanPeriod);
            statementSet.setDouble(5, outstanding_balance);
            statementSet.setDate(6, due_date);
            statementSet.setDouble(7, monthly_payment);

            statementSet.executeUpdate();

            // Get the generated loan_id
            ResultSet generatedKeys = statementSet.getGeneratedKeys();
            if (generatedKeys.next()) {
               int id = generatedKeys.getInt(1);  // Get the generated loan_id
               System.out.println("Your loan ID is " + id);
               System.out.println("Recommended monthly payment :" + monthly_payment);
   ...
   static void repayLoan() {
      try (Connection conn = DriverManager.getConnection(URL, USER, PASSWORD)) {
         Scanner scanner = new Scanner(System.in);
         String query = "SELECT loan_id, outstanding_balance, monthly_payment FROM Loans WHERE user_id = ? AND status = \"active\"";
         PreparedStatement statement = conn.prepareStatement(query);
         statement.setInt(1, userId);
         ResultSet loanList = statement.executeQuery();

         //listing the loans that havent been repaid
         System.out.println("The loans that haven't been repaid:");
         boolean hasLoans = false;
         while (loanList.next()) {
            hasLoans = true;
            int loanId = loanList.getInt("loan_id");
            double loanBalance = loanList.getDouble("outstanding_balance");
            double monthly_payment = loanList.getDouble("monthly_payment");
            System.out.printf("Loan ID: %d | Outstanding Balance: %.2f | Recommended monthly payment: %.2f %n", loanId, loanBalance, monthly_payment);
         }
         ...
         //user select the loan that they wish to repay
         System.out.println("Select the loan ID you wish to repay (-1 to quit): ");
         int selectedLoanId = scanner.nextInt();
         ...
         // Query to get the outstanding balance for the loan
         query = "SELECT outstanding_balance FROM Loans WHERE user_id = ? AND loan_id = ?";
         statement = conn.prepareStatement(query);
         statement.setInt(1, userId);
         statement.setInt(2, selectedLoanId);
         ResultSet rs = statement.executeQuery();

         if (rs.next()) {
            System.out.println("Enter the repayment amount: ");
            double repayAmount = scanner.nextDouble();
            double outstanding_balance = rs.getDouble("outstanding_balance");

            // Case when the loan is fully repaid
            if (outstanding_balance <= repayAmount) {
               System.out.println("Your loan is fully repaid.");
               if (outstanding_balance < repayAmount) {
                     System.out.println("Overpayment refunded :" + (repayAmount - outstanding_balance));
               }

               // Update loan status to "repaid" and set outstanding balance to 0
               query = "UPDATE Loans SET status = 'repaid', outstanding_balance = 0 WHERE user_id = ?";
               statement = conn.prepareStatement(query);
               statement.setInt(1, userId);
               statement.executeUpdate(); // executeUpdate() for updates
               creditTransaction(outstanding_balance, "Loan repayment | Loan id : "+selectedLoanId);


            } else {
               // Case when the loan is partially repaid
               // Update the outstanding balance with the new balance
               query = "UPDATE Loans SET outstanding_balance = outstanding_balance - ? WHERE user_id = ?";
               statement = conn.prepareStatement(query);
               statement.setDouble(1, repayAmount);
               statement.setInt(2, userId);
               statement.executeUpdate(); // executeUpdate() for updates

               System.out.println("Payment successful!");
               System.out.println("Your remaining balance is: " + (outstanding_balance-repayAmount));
               creditTransaction(repayAmount, "Loan repayment - Loan id : "+selectedLoanId);
               creditLoan();
            }

         } else {
            System.out.println("No loan found for the specified Loan ID.");
            repayLoan();
         }
   ...
   public static void creditLoan(){
      ...
      System.out.println("Enter your action (-1 to quit)");
   ...
   static void ban(){
      System.out.println("YOU HAVE BEEN BANNED FORM USING CREDIT OR DEBIT DUE TO OVERDUE LOANS");
      showTransactionMenu();
